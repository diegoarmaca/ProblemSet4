...
treated_all_cohorts <- select(treated_all_cohorts, state, leaid_10_11, 
                           ...
                           sy201112sig_model, sy201213sig_model, sy201314sig_model)
...
colnames(treated_all_cohorts) <- c("state", "lea_id","lea_name", 
                                   "ncessch","school_name",
                                   "model_2011_12","model_2012_13","model_2013_14")
treated_all_cohorts
```
```{r echo=FALSE, include=FALSE, message=FALSE}
...
```
```{r echo=FALSE, include=FALSE, message=FALSE}
### Treated dataset with all school years
results_treated_all <- merge(treated_all_cohorts, merged_math, by= "ncessch")
results_treated_all <- filter(results_treated_all,
...
results_treated_all <- select(results_treated_all, ncessch, schnam09, stnam.x, 
                          leaid.x, leanm.x, 
                          all_mth00pctprof_0910,all_mth00pctprof_1011, 
                          all_mth00pctprof_1112,all_mth00pctprof_1213,
                          all_mth00pctprof_1314)
colnames(results_treated_all) <- c("ncessch", "school_name", "state", "lea_id",
                               "lea_name", "scores_0910","scores_1011","scores_1112","scores_1213", "scores_1314")

### Create Treatment DataSet with only 2009-10 and 2013-14###
results_treated <- filter(results_treated_all,
                          !grepl("(([0-9]+[-][0-9]+)|([a-zA-Z]+[0-9]+)|([a-zA-z]+))",
                                 scores_0910) )
results_treated <- filter(results_treated, 
                          !grepl("(([0-9]+[-][0-9]+)|([a-zA-Z]+[0-9]+)|([a-zA-z]+))",
                                 scores_1314) )
results_treated <- select(results_treated, ncessch, school_name, state, 
                          lea_id, lea_name, scores_0910, scores_1314)
...
sapply(diff_in_diff_treated[,7], function(x) as.character(x))
diff_in_diff_treated[,7] <- 
...
```
```{r echo=FALSE, include=FALSE, message=FALSE}
### Create Control Group with all school years###
merged_math_all <- merged_math
unique_treated_schools <- melt(treated_schools, measure = 10:13)
unique_treated_schools <- unique(unique_treated_schools$value)
merged_math_all <- filter(merged_math, !(ncessch %in% 
                                as.numeric(unique_treated_schools)))
merged_math_all <- filter(merged_math_all,
                          !grepl("(([0-9]+[-][0-9]+)|([a-zA-Z]+[0-9]+)|([a-zA-z]+))",
                                 all_mth00pctprof_0910) )
merged_math_all <- filter(merged_math_all,
                          !grepl("(([0-9]+[-][0-9]+)|([a-zA-Z]+[0-9]+)|([a-zA-z]+))",
...
unique_treated_schools <- melt(treated_schools, measure = 10:13)
unique_treated_schools <- unique(unique_treated_schools$value)
merged_math_cleaned <- select(merged_math_all, all_mth00pctprof_0910, all_mth00pctprof_1314)
merged_math_cleaned <- filter(merged_math_all, !(ncessch %in% 
                                as.numeric(unique_treated_schools)))
...
results_control_all <- data.frame()
for (i in unique(results_treated$state)) {
    count_state <- count(filter(results_treated, state == i))
    filter_by_state <- which(merged_math_all$stnam.x == i)
    #set.seed(123)
    sample_by_state <- merged_math_all[sample(filter_by_state,
                                                  as.numeric(count_state)),]
    results_control_all <- rbind(results_control_all, sample_by_state)
    }
colnames(results_control_all) <- c("ncessch", "school_name", "state", "lea_id",
                               "lea_name", "scores_0910","scores_1011","scores_1112","scores_1213", "scores_1314")


results_control1 <- data.frame()
for (i in unique(results_treated$state)) {
    count_state <- count(filter(results_treated, state == i))
    ...
    sample_by_state <- merged_math_cleaned[sample(filter_by_state,
                                                  as.numeric(count_state)),]
    results_control1 <- rbind(results_control1, sample_by_state)
    }
colnames(results_control1) <- c("ncessch", "school_name", "state", "lea_id",
                               "lea_name", "scores_0910", "scores_1314")
results_control1 <- results_control1[,1:7]
...
colnames(results_control2) <- c("ncessch", "school_name", "state", "lea_id",
                               "lea_name", "scores_0910", "scores_1314")
results_control2 <- results_control2[,1:7]

diff_in_diff_controled1 <- melt(results_control1, measure = 6:7)
diff_in_diff_controled1[,7] <- 
sapply(diff_in_diff_controled1[,7], function(x) as.character(x))
diff_in_diff_controled1[,7] <- 
sapply(diff_in_diff_controled1[,7], function(x) as.numeric(x))
colnames(diff_in_diff_controled1) <- c("ncessch", "school_name", "state", "lea_id",
                               "lea_name", "cohort","percentage")

diff_in_diff_controled2 <- melt(results_control2, measure = 6:7)
diff_in_diff_controled2[,7] <- 
sapply(diff_in_diff_controled2[,7], function(x) as.character(x))
diff_in_diff_controled2[,7] <- 
...
#create fake treatment group
results_treated_fake <- data.frame()
for (i in unique(results_treated$state)) {
    count_state <- count(filter(results_treated, state == i))
    filter_by_state <- which(merged_math_cleaned$stnam.x == i)
    #set.seed(123)
    sample_by_state <- merged_math_cleaned[sample(filter_by_state,
                                                  as.numeric(count_state)),]
    results_treated_fake <- rbind(results_treated_fake, sample_by_state)
    }
colnames(results_treated_fake) <- c("ncessch", "school_name", "state", "lea_id",
                               "lea_name", "scores_0910", "scores_1314")
results_treated_fake <- results_treated_fake[,1:7]

diff_in_diff_treated_fake <- melt(results_treated_fake, measure = 6:7)
diff_in_diff_treated_fake[,7] <- 
sapply(diff_in_diff_treated_fake[,7], function(x) as.character(x))
diff_in_diff_treated_fake[,7] <- 
...
colnames(diff_in_diff_treated_fake) <- c("ncessch", "school_name", "state", "lea_id",
                               "lea_name", "cohort","percentage")
diff_in_diff_treated_fake <- filter(diff_in_diff_treated_fake, cohort == 
                                 "scores_0910" | cohort == "scores_1314")
```
...
diff_in_diff_controled1 <- mutate(diff_in_diff_controled1, sig_program = "0")
diff_in_diff_controled2 <- mutate(diff_in_diff_controled2, sig_program = "0")
diff_in_diff_both1 <- rbind(diff_in_diff_controled1, diff_in_diff_treated)
diff_in_diff_both2 <- rbind(diff_in_diff_controled2, diff_in_diff_treated)
diff_in_diff_both_fake <- rbind(diff_in_diff_controled1, diff_in_diff_treated_fake)
```

# Summary Statistics

The distribution of the treatment group before and after the program implementation is shown in Figure 2. The mean of the proficiency rate in both school years remained under 40%. The maximum proficiency rate was 93% in the school year 2009-2010. The lowest proficiency rate was 2% in school year 2013-2014. The distribution of the control group is shown in Figure 3. In the control group, the mean of the proficiency rate across the five school years was above 60%. The maximum proficiency rate was 98% in both school years 2009-2010 and 2013-2014. The lowest proficiency rate was 5% in the school year 2013-2014. 

```{r echo=FALSE, include=TRUE, message=FALSE, fig2, fig.height = 5, fig.width = 7}
diff_in_diff_boxplot <- rbind(diff_in_diff_treated, diff_in_diff_controled1)
boxplot(percentage ~ cohort+sig_program, data = diff_in_diff_boxplot,
        xlab = "School Year", 
        ylab = "Proficiency Rate", main = "Control Group and Treatment Group Boxplot")
```
...
results_control_all[,6:10] <- sapply(results_control_all[,6:10], function(x) as.character(x))
results_control_all[,6:10] <- sapply(results_control_all[,6:10], function(x) as.numeric(x))
results_control_plot <- melt(results_control_all, measure = 6:10)
...
results_treated_all[,6:10] <- sapply(results_treated_all[,6:10], function(x) as.character(x))
results_treated_all[,6:10] <- sapply(results_treated_all[,6:10], function(x) as.numeric(x))
results_treated_plot <- melt(results_treated_all, measure = 6:10)
...
```{r echo=FALSE, include=TRUE, message=FALSE, fig6, fig.height = 4, fig.width = 7}
diff_in_diff_both1 %>% 
  ggplot(aes(x = cohort,
             y = percentage,
             color = sig_program
  ...
  geom_point() +
  geom_line(aes(group = ncessch), alpha = .2) +
  theme_minimal() +
  labs(x = "Cohorts",
       y = "Percentage",
       color = "SIG pogram") +
...
model_control1 <- filter(diff_in_diff_both1, cohort %in% c("scores_0910","scores_1314"))
model_control1$cohort <- if_else(model_control1$cohort == "scores_0910", "0","1")
model_control1$sig_program <- as.factor(model_control1$sig_program)
model_control1$cohort <- as.factor(model_control1$cohort)
model_control1 <- 
  rename(model_control1, 
    year = cohort
    )
diff_in_diff_model_control1 <- lm(percentage ~  sig_program*year, 
                         data = model_control1)
summ(diff_in_diff_model_control1, model.info = FALSE, model.fit = FALSE)
```
...
To validate the model, we perform the diff-in-diff analysis with another comparison group to see if we would obtain similar estimates of the impact of the program. As shown in Table 2, the estimate of [something] is not too far away from [something] in Model 1. 

```{r echo=FALSE, include=TRUE, message=FALSE}
model_control2 <- filter(diff_in_diff_both2, cohort %in% c("scores_0910","scores_1314"))
model_control2$cohort <- if_else(model_control2$cohort == "scores_0910", "0","1")
model_control2$sig_program <- as.factor(model_control2$sig_program)
model_control2$cohort <- as.factor(model_control2$cohort)
model_control2 <- 
  rename(model_control2, 
    year = cohort
    )
diff_in_diff_model_control2 <- lm(percentage ~  sig_program*year, 
                         data = model_control2)
summ(diff_in_diff_model_control2, model.info = FALSE, model.fit = FALSE)
```
Table 2. OLS for diff-in-diff between 2009-10 and 2013-2014 with another set of control group

### Model 3: Model with fake treatment group

Another validation method we used was to test the model on a fake treatment group that should not be affected by the program. As shown in Table 3, the estimate of difference is very small and p-value is insignificant. 

```{r echo=FALSE, include=TRUE, message=FALSE}
model_treatment_fake <- filter(diff_in_diff_both_fake, cohort %in% c("scores_0910","scores_1314"))
model_treatment_fake$cohort <- if_else(model_treatment_fake$cohort == "scores_0910", "0","1")
model_treatment_fake$sig_program <- as.factor(model_treatment_fake$sig_program)
model_treatment_fake$cohort <- as.factor(model_treatment_fake$cohort)
model_treatment_fake <- 
  rename(model_treatment_fake, 
    year = cohort
    )
diff_in_diff_model_both_fake <- lm(percentage ~sig_program*year, 
                         data = model_treatment_fake)
summ(diff_in_diff_model_both_fake, model.info = FALSE, model.fit = FALSE)
```
...
ggplot(results_control1, aes(x=state)) +
  geom_bar()+
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
...
Paul J. Gertler, Sebastian Martinez, Patrick Premand, Laura B. Rawlings & Christel M. J.                               Vermeersch, 2016. "Impact Evaluation in Practice, Second Edition," World Bank Publications, The World Bank, number 25030, March. 
