...
results_control[,6:length(results_control)] <- 
sapply(results_control[,6:length(results_control)], function(x) as.numeric(x))
...
binded_data[,7] <- 
sapply(binded_data[,7], function(x) as.numeric(x))
...
for (i in unique(results_treated$state)) {
    ...
    sample_by_state <- merged_math_cleaned[sample(row.names(filter(merged_math_cleaned, 
                                                              stnam.x == i)),
