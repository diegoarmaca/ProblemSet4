...
The distribution of percentage rate in the treated group in school year 2009-10 and school year 2013-14 are shown in Figure 1. We can see that most of the schools in the treated group have proficiency rate under 50%, meaning a large portion of the students among these schools don’t meet the proficient level in math assessment.   
...
diff_in_diff_treated <- filter(diff_in_diff_treated, cohort == "scores_0910" | cohort == "scores_1314")
```
```{r echo=FALSE, include=FALSE, message=FALSE}
...
The distribution of the treatment group before and after the program implementation is shown in Figure 2. The mean of the proficiency rate in both school years remain under 40%. The maximum proficiency rate is 93% in the school year 2009-2010. The least proficient rate is 2% in school year 2013-2014.  
...
The distribution of the control group is shown in Figure 3. In the control group, the mean of the proficiency rate across five school years are above 60%. The maximum proficiency rate is 98% in both years 2009-2010 and 2013-2014. The least proficient rate is 5% in school year 2013-2014.  
...
To assure the comparability of the treatment group and the control group, we make sure that the two groups have the composition in the distribution of schools across states. The distribution of schools across states in treatment group is shown in Figure 4, and the control group distribution is shown in Figure 5. 
...
```
Figure 4. Distribution of schools who implemented SIG models across states 
...
ggplot(results_control, aes(x=state)) +
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
Figure 5. Distribution of schools who did not implemented SIG models across states 

We plot the math proficiency rate of each school in both treatment and control groups. As shown in Figure 6, there is no clear trend in either group. 
...
```
Figure 6. Math proficiency rate of each school in both treatment and control groups 
...
$$ Y_{i,t} = \beta0 + \beta1 \text{(Treatment group dummy)}_{i} + \beta2 \text{(Time dummy)}_{t} +  
\beta3 (Treatment group dummy × Time dummy)_{i,t} + \epsilon_{i,t}$$
...
### Model 1 
Model 1 observes the change between school year 2009-10 and 2010-2011. As the result shown in Table 1, the p-value of $\beta3$ is 0.37, the effect of SIG program on math proficiency rate is insignificant.  
