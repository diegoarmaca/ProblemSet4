...
results_treated$scores_1011 <- as.numeric(results_treated$scores_1011)
results_treated$scores_1112 <- as.numeric(results_treated$scores_1112)
results_treated$scores_1213 <- as.numeric(results_treated$scores_1213)
...
```
```{r include=TRUE}
binded_data <- melt(results_treated, measure = 6:10)
...
merged_math_cleaned <- filter(merged_math, !(ncessch %in% 
                                as.numeric(treated_schools$ncessch_1011)))

merged_math_cleaned <- filter(merged_math,
                          !grepl("([0-9]+[-][0-9]+|[a-zA-Z]+[0-9]+|[a-zA-z]+)",
                                 all_mth00pctprof_0910) )
...
for (i in unique(results_treated$state)) {
    ...
    #sample_by_state <- sample(merged_math_cleaned[
    #  nrow(filter(merged_math_cleaned, stnam.x == "FLORIDA")),],i)
    sample_by_state <- merged_math_cleaned[sample(nrow(filter(merged_math_cleaned, 
                                                              stnam.x == i)),
...
ggplot(results_control, aes(x=stnam.x)) +
  geom_bar()
